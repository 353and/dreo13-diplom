...
# Списки для реалистичных названий исполнителей
...
# Генерируем уникальных исполнителей (до 50)
artists_pool = []
for _ in range(70):
...
# Добавляем исполнителей и треки в БД
artist_cache = {}
track_count = 0
...
if not test_user:
    ...
    db.add(test_user)
    print("Создан тестовый пользователь (id=1, username='test', пароль='password')")
else:
...
if not admin:
    ...
    db.add(admin)
    print("Создан администратор (username='admin', пароль='admin123')")
else:
    print("Администратор уже существует")

db.commit()

# --- Генерация синтетических взаимодействий для тестового пользователя ---
all_tracks_db = db.query(Track).all()
track_ids = [t.id for t in all_tracks_db]
user_ids = [1]  # только тестовый пользователь
...
# --- Обучение ALS модели на всех взаимодействиях ---
all_interactions = db.query(Interaction).all()
# Собираем всех пользователей, у которых есть взаимодействия
